...
from components import CalculateApplicationGain, ShowResults
...
def subclassDefine():
    CalculateApplicationGain.register(SavingAccount)
    CalculateApplicationGain.register(InvestmentAccount)
    ShowResults.register(CheckingAccount)
    ShowResults.register(SavingAccount)
    ShowResults.register(InvestmentAccount)
        ...
        if option == "1":
            ...
            if account and account.getAccountType() == "poupança":
                ...
                if time > 0:
                    ...
                    account.showResults("Rendimento estimado", account.calculateDifference(gain))
                else:
            ...
            if account and account.getAccountType() == "investimento":
                account.investment()
                gain = account.calculateGain(1)
                account.showResults("Resultado do investimento", account.calculateDifference(gain))
                account.setBalance(gain)
            else:
