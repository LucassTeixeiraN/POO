...
from post import post_image, post_text, post_video
...
def main():
    ...
    while True:
        ...
        if option == '1':
            ...
            user: User = network.find_user(username)
            if isinstance(user, str):
                print(user)
            else:
                post_type = input("Escolha o tipo de post (imagem/video/texto): ").strip().lower()
                message = input("Digite a mensagem: ")

                if post_type == 'imagem':
                    result = user.post_message(message, post_image)
                elif post_type == 'video':
                    result = user.post_message(message, post_video)
                else:
                    result = user.post_message(message, post_text)
