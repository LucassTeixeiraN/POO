...
def main():
    ...
    for i, invoice in enumerate(produtos, start=1):
        print(f"\nFatura {i}:")
        print(f"Número do Item: {invoice.__numeroItem}")
        print(f"Descrição: {invoice.__descricaoItem}")
        print(f"Quantidade: {invoice.__quantidadeComprada}")
        print(f"Preço Unitário: R${invoice.__precoItem:.2f}")
